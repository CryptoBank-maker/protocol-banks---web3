    ...
    try {
      ...
      if (!options?.skipSecurityCheck) {
        const securityResult = await performCheck({
          ...
          from: "", // Will be filled by the check
          tokenAddress: "",
        ...
        if (securityResult) {
          warnings = securityResult.allWarnings || []
          const isHighRisk = securityResult.overallRisk === "high" || securityResult.overallRisk === "critical"
          setState(prev => ({ 
            ...prev, 
            securityWarnings: warnings,
            requiresBiometric: isHighRisk || payment.amount > 1000
          }))

          if (!securityResult.canProceed) {
            setState(prev => ({ ...prev, step: "failed", error: "Transaction blocked due to security concerns" }))
            await logAction("PAYMENT_FAILED", "payment", payment.recipient, { payment, reason: "security_check_failed" })
      ...
      await logAction("PAYMENT_INITIATED", "payment", payment.recipient, {
