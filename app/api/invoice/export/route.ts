    ...
    <html>
      <head>
        ...
        <style>
          ...
          * { margin: 0; padding: 0; box-sizing: border-box; }
          ...
          .print-btn { display: block; margin: 16px auto; padding: 10px 24px; background: #000; color: white; border: none; border-radius: 8px; font-size: 14px; font-weight: 600; cursor: pointer; }
      ...
      <body>
        <div class="page">
          ...
          <div class="header">
            <div>
              <div class="logo">Protocol <span>Bank</span></div>
          ...
          <div class="payment-info">
            ...
            <div class="payment-row">
              <span class="payment-label">Network</span>
               <span class="payment-value">${invoice.chain || (invoice.chain_id ? `Chain ${invoice.chain_id}` : 'Base')}</span>
            </div>
            ${invoice.paid_by ? `
            <div class="payment-row">
          ...
          <!-- Footer -->
          <div class="footer">
            <p>This invoice was generated by Protocol Bank.</p>
            <p>All payments are non-custodial and settled directly on the blockchain.</p>
            <p style="margin-top: 8px;">protocol-bank.vercel.app</p>
          </div>
        </div>
      </body>
    </html>
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Auth Check 2:
    if (invoice && invoice.signature && invoice.signature !== signature) {
    ...
    if (!invoice && process.env.NEXT_PUBLIC_ALLOW_DEMO_MODE !== 'false') {
      invoice = {
        ...
        merchant_name: 'Protocol Bank Test',
